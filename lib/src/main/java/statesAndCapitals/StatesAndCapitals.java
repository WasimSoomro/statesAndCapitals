...
public class StatesAndCapitals
{
    ...
    public void testStatesAndCapitals() throws IOException
    {
        ...
        StateInfo cardinalState = states.stream().filter(s -> "cardinal".equals(s.getStateBird())).findAny().orElseThrow();
        ...
        Boolean isAnyStateLessThan0Elevation = states.stream().anyMatch(s -> s.getLowestElevation() < 0);
        ...
        Boolean isAnyStateGreaterThan21000Elevation = states.stream().anyMatch(s -> s.getHighestElevation() > 21000);
        ...
        Boolean doNoStatesHaveAOneWordMotto = states.stream().noneMatch(s -> s.getMotto().split("\\s+").length == 1);
        ...
        Double averageYearlyPrecipitationAcrossStateCapitals = states.stream().map(StateInfo::getCapital).collect(Collectors.averagingDouble(CapitalInfo::getAverageYearlyPrecipitationInInches));
        ...
        Integer totalYearlyPrecipitationAcrossStateCapitals = states.stream().map(StateInfo::getCapital).collect(summingInt(CapitalInfo::getAverageYearlyPrecipitationInInches));
        ...
        Map<String, Long> numberOfStatesByTimeZone = states.stream().collect(Collectors.groupingBy(state -> state.getTimeZones().toString(), counting()));
        ...
        Map<String, Long> numberOfStateCapitalsByTimeZone = states.stream().collect(Collectors.groupingBy(s -> s.getCapital().getTimeZone(), Collectors.counting()));
        ...
        String allStateNamesSemicolonDelimited = states.stream().map(StateInfo::getStateName).collect(Collectors.joining("; "));
        ...
        List<String> allDistinctStateBirdsMinusMockingbirds = states.stream().map(StateInfo::getStateBird).filter(b -> !b.contains("mockingbird")).distinct().collect(Collectors.toList());
